...
class nlp_processor:
    ...
    def __init__(
        ...
        self.files_setup.generate_metadata_file(
            self.data_path, self.metadata_addt_column_names
        )
    ...
    def refresh_object_metadata(self):
        ...
        self.metadata = self.files_setup.generate_metadata_file(
            self.data_path, self.metadata_addt_column_names
        )  # make sure text_id added

    def sync_local_metadata(self):
        "update the metadata local file to reflect actual state of files in the data directory"
        self.metadata = self.files_setup.refresh_local_metadata(
            self.metadata, self.data_path
        )
    ...
    def download_text_id(self, text_ids):
        ...
        for text_id in text_ids:
            ...
            web_filepath = self.metadata.loc[
                lambda x: x.text_id == text_id, "web_filepath"
            ].values[0]
            tmp_metadata = self.files_setup.download_document(
                self.metadata, self.data_path, text_id, web_filepath
            )
    ...
    def clear_directories(
        ...
        self.files_setup.clear_directories(
            self,
            raw_files,
            txt_files,
    ...
    def convert_to_text(self, text_ids, force_ocr=False):
        ...
        for text_id in text_ids:
            ...
            tmp_metadata = self.files_setup.convert_to_text(
                self.metadata,
                self.data_path,
                text_id,
                self.windows_tesseract_path,
                self.windows_poppler_path,
                force_ocr,
            )
    ...
    def transform_text(
        ...
        for text_id in text_ids:
            ...
            text_path = self.metadata.loc[
                lambda x: x.text_id == text_id, "local_txt_filepath"
            ].values[0]
            language = self.metadata.loc[
                lambda x: x.text_id == text_id, "detected_language"
            ].values[0]
            ...
            if (".txt" in str(text_path)) & (
                not (
                    os.path.exists(
                        f"{self.data_path}transformed_txt_files/{path_prefix}{text_id}.txt"
                    )
                )
            ):
                ...
                if perform_remove_multiple_header_and_footers:
                    stringx = self.text_transformation.remove_headers_and_footers(
                        stringx
                    )
                ...
                if replace_words_with_punctuation_df is not None:
                    stringx = self.text_transformation.replace_words_text_transform(
                        stringx, replace_words_with_punctuation_df
                    )
                ...
                if replace_words_df is not None:
                    stringx = self.text_transformation.replace_words_text_transform(
                        stringx, replace_words_df
                    )
                if exclude_words_df is not None:
                    stringx = self.text_transformation.exclude_words(
                        stringx, exclude_words_df
                    )
                if perform_remove_stopwords:
                    stringx = self.text_transformation.remove_stopwords(
                        stringx, language
                    )
                if perform_stemming:
    ...
    def gen_word_count_csv(self, text_ids, path_prefix, exclude_words):
        ...
        if os.path.exists(csv_path):
            ...
            csv = pd.DataFrame(
                {"text_id": self.metadata.text_id.values, "word_count_dict": ""}
            )
        ...
        for text_id in text_ids:
            ...
            if os.path.exists(text_path) & (
                (str(prior_value) == "") | (str(prior_value) == "nan")
            ):
                # reading original text
                file = open(f"{text_path}", "r", encoding="UTF-8")
                stringx = file.read()
                ...
                word_dict = self.text_transformation.gen_word_count_dict(
                    stringx, exclude_words
                )
    ...
    def bar_plot_word_count(self, text_ids, path_prefix, n_words=10, title=""):
        ...
        if os.path.exists(csv_path):
            csv = (
                ...
                .loc[lambda x: x.text_id.isin(text_ids), :]
    ...
    def gen_sentiment_report(
        self, text_id=None, stringx=None, sentiment_analyzer=SentimentIntensityAnalyzer
    ):
        ...
        return self.text_transformation.gen_sentiment_report(
            stringx=stringx, sentiment_analyzer=sentiment_analyzer
        )

    def gen_sentiment_csv(
        self,
        text_ids,
        path_prefix,
        ...
        for text_id in text_ids:
            ...
            prior_value = csv.loc[
                csv.text_id == text_id, "avg_sentiment_w_neutral"
            ].values[0]
            if os.path.exists(txt_path) & (
                ((str(prior_value) == "") | (str(prior_value) == "nan")) | overwrite
            ):
                # reading original text
                file = open(f"{txt_path}", "r", encoding="UTF-8")
                stringx = file.read()
                ...
                sentiments = [
                    self.text_transformation.get_single_sentiment(
                        x, sentiment_analyzer
                    )["compound"]
                    for x in stringx.split("|")
                    if (len(x) > 3) & (len(x.split(" ")) > 2) & (not (x.isnumeric()))
                ]  # only do sentiment for sentences with more than 2 words and not numeric

                # adding and writing to CSV
                try:
                    csv.loc[csv.text_id == text_id, "avg_sentiment_w_neutral"] = sum(
                        sentiments
                    ) / len(sentiments)
                    csv.loc[csv.text_id == text_id, "avg_sentiment_wo_neutral"] = sum(
                        [x for x in sentiments if x != 0.0]
                    ) / len([x for x in sentiments if x != 0.0])
                    csv.loc[csv.text_id == text_id, "neutral_proportion"] = len(
                        [x for x in sentiments if x == 0.0]
                    ) / len(sentiments)
                except:
    ...
    def plot_word_occurrences(
        self, text_ids_list, word, path_prefix, x_labels=None, title=""
    ):
        ...
        if os.path.exists(csv_path):
            ...
            p, plot_df = self.visualizations.plot_word_occurrences(
                csv, text_ids_list, word, x_labels, title
            )
    ...
    def plot_sentiment(
        ...
        if os.path.exists(csv_path):
            csv = pd.read_csv(csv_path)
            p, plot_df = self.visualizations.plot_sentiment(
                csv, text_ids_list, x_labels, title, sentiment_col
            )
            return (p, plot_df)
    ...
    def gen_summary_stats_csv(self, text_ids, path_prefix):
        ...
        for text_id in text_ids:
            ...
            orig_txt_path = self.metadata.loc[
                lambda x: x.text_id == text_id, "local_txt_filepath"
            ].values[0]
            ...
            if os.path.exists(txt_path) & (
                ...
                # calculating n_words
                n_words = len(
                    [x for x in stringx.split(" ") if len(x) > 1]
                )  # minimum word length
                ...
                n_sentences = len(
                    [x for x in stringx.split("|") if len(x) > 2]
                ...
                avg_word_length = [
                    len(x) for x in orig_stringx.split(" ") if len(x) > 1
                ]
                try:
                ...
                avg_word_incidence = [
                    self.text_transformation.get_word_frequency(x, language)
                    for x in word_list
                ]
                try:
                    avg_word_incidence = sum(avg_word_incidence) / len(
                        avg_word_incidence
                    )
                except:
                ...
                numeric_proportion = num_chars_numeric / (
                    num_chars_numeric + num_chars_alpha
                )
                ...
                csv.loc[csv.text_id == text_id, "avg_word_incidence"] = (
                    avg_word_incidence
                )
                ...
                csv.loc[csv.text_id == text_id, "numeric_proportion"] = (
                    numeric_proportion
                )
                csv.to_csv(csv_path, index=False)
    ...
    def plot_summary_stats(
        ...
        if os.path.exists(csv_path):
            csv = pd.read_csv(csv_path)
            p, plot_df = self.visualizations.plot_summary_stats(
                csv, text_ids_list, x_labels, title, summary_stats_col
            )
            return (p, plot_df)

    def plot_text_similarity(
        self, text_ids, path_prefix="", label_column="text_id", figsize=(22, 16)
    ):
        ...
        """
        return self.visualizations.gen_similarity_plot(
            self, text_ids, path_prefix, label_column, figsize
        )
    ...
    def gen_entity_count_csv(self, text_ids):
        ...
        if os.path.exists(csv_path):
            ...
            csv = pd.DataFrame(
                {"text_id": self.metadata.text_id.values, "entity_count_dict": ""}
            )
        ...
        for text_id in text_ids:
            ...
            if os.path.exists(text_path) & (
                ...
                lang = self.metadata.loc[
                    lambda x: x.text_id == text_id, "detected_language"
                ].values[0]

                try:
                    entity_dict = self.text_transformation.gen_entity_count_dict(
                        stringx, lang
                    )
                except:
    ...
    def train_bertopic_model(
        self, text_ids, model_name, notes="", split_by_n_words=None
    ):
        ...
        """
        self.text_transformation.train_bertopic_model(
            self, text_ids, model_name, notes, split_by_n_words
        )
    ...
    def visualize_bertopic_model(
        ...
        **kwargs,
        ...
        plot_df = self.text_transformation.bertopic_visualize(
            self,
            model,
            model_name,
            method_name,
        ...
        print(
            f"plot saved to {self.data_path}bertopic_models/{model_name}/{plot_name}.*"
        )
        return plot_df
    ...
    def replace_words(self, text_ids, replacement_list, path_prefix=""):
        ...
        """
        self.text_transformation.replace_words(
            self, text_ids, replacement_list, path_prefix
        )
    ...
    def gen_search_terms(
        ...
        parameters:
            ...
            :match_exclusion: Pandas DataFrame: optional dataframe with two columns, "permutation", and "sentence_context". Permutation contains the word/term to search for and sentence_context contains the sentence_context string to exclude from matches. Use this in case you don't want certain matches to be counted as a match (deemed irrelevant, etc.)
        output:
        ...
        """
        self.search_terms.gen_search_terms(
            self,
            group_name,
            text_ids,
            search_terms_df,
    ...
    def gen_aggregated_search_terms(
        self, group_names, text_ids, search_terms_df, path_prefix, character_buffer=100
    ):
        ...
        """
        self.search_terms.gen_aggregated_search_terms(
            self,
            group_names,
            text_ids,
            search_terms_df,
    ...
    def gen_co_occurring_terms(self, group_name, co_occurrence_terms_df, n_words=50):
        ...
        """
        self.search_terms.gen_co_occurring_terms(
            self, group_name, co_occurrence_terms_df, n_words
        )
    ...
    def gen_second_level_search_terms(self, group_name, second_level_search_terms_df):
        ...
        """
        self.search_terms.gen_second_level_search_terms(
            self, group_name, second_level_search_terms_df
        )
    ...
    def gen_top_words(
        ...
        """
        self.search_terms.gen_top_words(
            self, group_names, text_ids, path_prefix, per_1000, top_n, exclude_words
        )
